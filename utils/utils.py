...
def get_optimal_action(
    ...
    """
    Interpolate the optimal action for a given state using barycentric coordinates.

    Computes a weighted average of the optimal actions at the surrounding grid
    vertices, using barycentric weights as coefficients.

    Returns:
        (optimal_action, lambdas)
    """
    ...
    neighbor_actions = optimal_policy.action_space[best_action_indices]

    # Barycentric interpolation of action values
    optimal_action = lambdas @ neighbor_actions

    return optimal_action, lambdas

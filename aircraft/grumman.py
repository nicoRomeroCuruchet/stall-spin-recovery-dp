...
class Grumman:
    ...
    def _cl_from_alpha(self, alpha, elevator, q_hat):
        ...
        The calculation is derived from the angle of attack (α), 
        elevator deflection, and pitch rate (q_hat).
        """
        # Convert everything to ndarrays and broadcast shapes
        ...
        # Masks for the three α regions
        mask_low = alpha <= self.ALPHA_NEGATIVE_STALL
        mask_high = alpha >= self.ALPHA_STALL
        mask_mid = ~(mask_low | mask_high)

        # Allocate output array
        c_lift = np.empty_like(alpha)

        # Region 1: α below negative-stall threshold
        c_lift[mask_low] = self.CL_0 + self.CL_ALPHA * self.ALPHA_NEGATIVE_STALL

        # Region 2: α above positive-stall threshold
        c_lift[mask_high] = self.CL_0 + self.CL_ALPHA * self.ALPHA_STALL
        
        # (uncomment next line for the "opposite slope" stall model)
        # c_lift[mask_high] = (
        #     -self.CL_ALPHA * alpha[mask_high]
        #     + self.CL_0 
        #     + 2 * self.CL_ALPHA * self.ALPHA_STALL
        # )

        # Region 3: linear section with control derivatives
        c_lift[mask_mid] = (
            self.CL_0
            + self.CL_ALPHA * alpha[mask_mid]
            + self.CL_ELEVATOR * elevator[mask_mid]
            + self.CL_QHAT * q_hat[mask_mid]
        )

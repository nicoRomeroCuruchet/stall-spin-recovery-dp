...
from PolicyIterationStall import PolicyIterationStall, PolicyIterationStallConfig
...
def generate_paper_heatmap_figures(pi: PolicyIterationStall, prefix: str) -> None:
    ...
    for i, v_target in enumerate(v_targets):
        ...
        if i == 0: ax_de.set_title('Policy for Elevator', pad=10)
...
def validate_with_casadi(pi: PolicyIterationStall, prefix: str) -> None:
    ...
    for step in range(1500):
        ...
        if has_dived and new_gamma >= 0.0:
            hist["t"].append(t);     hist["gamma"].append(new_gamma)
            hist["v_norm"].append(obs[1]); hist["alpha"].append(obs[2])
            hist["q"].append(obs[3]);      hist["de"].append(action[0])
            hist["dt_ctrl"].append(action[1]); hist["h"].append(h)
            break
